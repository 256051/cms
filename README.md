...
新增 n8n 资料整理与写作工作流，复用 CMS 数据库 AI 配置并保存为草稿，附独立本地演示，见 [n8n 自动化](docs/n8n.md)。

新增 AI 写作助手：数据库加密保存 API 地址、密钥和模型，支持连接测试、按主题写作、摘要、标题、标签、翻译与润色，生成结果确认后应用到草稿，见 [AI 写作助手](docs/ai-writing.md)。

新增数字商品上架、立即购买、交易订单、私有文件下载，以及可分别配置的支付宝、微信支付和 Stripe，见 [数字商品与收款](docs/commerce.md)。
