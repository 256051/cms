...
schema 9 新增后台备份、定时执行、失败状态和逻辑备份包恢复，见 [内容运营完善](editorial-enhancements.md#备份与明细维护)。以下原生数据库备份流程仍可使用。
...
主题选择、七套主题各自保存的设置和版本号都在数据库 `cms_theme` 表中，随数据库一并备份；主题代码和静态缩略图随 web 镜像发布。恢复后检查当前主题及各主题配置是否与备份时一致。
...
- **启动数据库报错**：检查 Provider 名称、地址、账号权限、库是否已建立。正常启动会先升级已有旧库，失败则退出；首次建站执行 `--initialize`，已有库可用 `--migrate` 单独排查升级。保留备份，修复权限或迁移失败原因后重启；不要删除版本记录或开启全局结构同步来绕过。
...
通知默认关闭；邮件与企业微信配置、重试规则和去重边界见 [主动通知配置](next-enhancements.md#主动通知配置)。后台“通知记录”仅向管理员开放，不返回渠道凭据。通知发送独立于定时发布，停用渠道的积压不会挡住其他渠道；真实投递需使用部署环境的凭据验证。

当前 schema 14，逻辑备份包含公共区块、旧地址、SEO、附件分组、业务字段、咨询表单和通知记录，兼容恢复 schema 9–14 的归档。恢复后还应核对产品／案例、旧链接跳转及咨询附加字段；演练实例应保持 `Notifications__Enabled=false`。
