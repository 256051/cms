    ...
    : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        if (header.Count != 1 || header[0] is not { } value ||
            !value.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase)) return AuthenticateResult.NoResult();
        var token = await tokens.AuthenticateAsync(value[7..]);
        if (token == null) return AuthenticateResult.Fail("Invalid access token");
        var claims = new List<Claim>
            { new(ClaimTypes.NameIdentifier, token.UserId), new("token_id", token.Id), new("token_name", token.Name) };
    ...
    protected override async Task HandleChallengeAsync(AuthenticationProperties properties)
    {
        ...
        await Response.WriteAsJsonAsync(new ApiResponse<object>("INVALID_ACCESS_TOKEN", "请通过 HTTPS 提供有效的访问令牌。", null,
    ...
    protected override async Task HandleForbiddenAsync(AuthenticationProperties properties)
    {
        ...
        await Response.WriteAsJsonAsync(new ApiResponse<object>("INSUFFICIENT_SCOPE", "访问令牌没有执行此操作的权限。", null,
            Context.TraceIdentifier));
    }
}