...
public sealed class CmsException(int status, string code, string message, int? retryAfterSeconds = null) : Exception(message)
{
...
public sealed class CmsRepository(IFreeSql db)
{
    ...
    public Task<T?> FindAsync<T>(string id) where T : Entity => db.Select<T>().Where(x => x.Id == id).FirstAsync()!;

    /// <summary>Find one matching entity.</summary>
    public Task<T?> FirstAsync<T>(Expression<Func<T, bool>> predicate) where T : Entity => db.Select<T>().Where(predicate).FirstAsync()!;

    /// <summary>List entities using a database predicate and stable order.</summary>
    public Task<List<T>> ListAsync<T>(Expression<Func<T, bool>>? predicate = null) where T : Entity => db.Select<T>().WhereIf(predicate != null, predicate!).OrderBy(x => x.CreatedAt).OrderBy(x => x.Id).ToListAsync();

    /// <summary>Count matching entities.</summary>
    public Task<long> CountAsync<T>(Expression<Func<T, bool>>? predicate = null) where T : Entity => db.Select<T>().WhereIf(predicate != null, predicate!).CountAsync();

    /// <summary>Query a stable newest-first page.</summary>
    public async Task<PageResult<T>> PageAsync<T>(Expression<Func<T, bool>> predicate, int page, int size, Expression<Func<T, object>>? sort = null) where T : Entity
    {
        page = Math.Max(1, page); size = Math.Clamp(size, 1, 100);
        var query = db.Select<T>().Where(predicate);
        var total = await query.CountAsync();
        var items = await query.OrderByDescending(sort ?? (x => x.CreatedAt)).OrderBy(x => x.Id).Page(page, size).ToListAsync();
        return new(items, total, page, size);
    }

    /// <summary>Insert inside the caller's repository transaction.</summary>
    public Task<int> InsertAsync<T>(T item) where T : Entity => db.Insert(item).ExecuteAffrowsAsync();

    /// <summary>Replace an existing entity.</summary>
    public Task<int> UpdateAsync<T>(T item) where T : Entity => db.Update<T>().SetSource(item).ExecuteAffrowsAsync();

    /// <summary>Delete an entity by identifier.</summary>
    public Task<int> DeleteAsync<T>(string id) where T : Entity => db.Delete<T>().Where(x => x.Id == id).ExecuteAffrowsAsync();

    /// <summary>Read only navigation fields of selected published content.</summary>
    public Task<List<Content>> MenuContentsAsync(string[] ids) => db.Select<Content>().Where(x => x.Published && ids.Contains(x.Id)).ToListAsync(x => new Content { Id = x.Id, Kind = x.Kind, Slug = x.Slug, PublishedTitle = x.PublishedTitle });
    ...
    public async Task<PageResult<Content>> MenuTargetsAsync(string kind, string query, int page)
    {
        ...
        var select = db.Select<Content>().Where(x => x.Published && x.Kind == kind && (query == "" || x.PublishedTitle.Contains(query)));
        var total = await select.CountAsync();
        var rows = await select.OrderByDescending(x => x.PublishedAt).OrderBy(x => x.Id).Page(page, 20).ToListAsync(x => new Content { Id = x.Id, Kind = x.Kind, Slug = x.Slug, PublishedTitle = x.PublishedTitle });
    ...
    public async Task SaveContentAsync(Content content, int expected)
    {
        ...
        var changed = await db.Update<Content>().SetSource(content).Where(x => x.Version == expected).ExecuteAffrowsAsync();
    ...
    public async Task InitializeSchemaAsync()
    {
        ...
        await db.Update<MenuItem>().Where(x => x.Type == "" || x.Type == null).Set(x => x.Type, "custom").ExecuteAffrowsAsync();
        ...
        await db.Update<SiteSettings>().Where(x => x.Id == "site")
            ...
            .Set(x => x.HomePageSize, 12).Set(x => x.CategoryPageSize, 12).Set(x => x.TagPageSize, 12).Set(x => x.SearchPageSize, 12)
