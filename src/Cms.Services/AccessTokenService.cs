...
public sealed class AccessTokenService(CmsRepository repository, TimeProvider clock)
{
    ...
    public async Task<PageResult<AccessTokenView>> ListAsync(int page)
    {
        ...
        return new PageResult<AccessTokenView>(
            result.Items.Select(x => View(x, users.GetValueOrDefault(x.UserId))).ToList(), result.Total, result.Page,
            result.PageSize);
    }

    /// <summary>Create an explicit bounded credential and audit the issuance atomically.</summary>
    public Task<IssuedAccessToken> CreateAsync(string actor, AccessTokenInput input)
    {
        return repository.WriteAsync(actor, "token.create", async repo =>
        {
            var now = clock.GetUtcNow();
            if (string.IsNullOrWhiteSpace(input.Name) || input.Name.Length > 100 ||
                input.Name.Any(c => char.IsControl(c) || c is '<' or '>') || input.UserId is not { Length: 32 } ||
                input.Scopes is not { Length: > 0 and <= 4 } ||
                input.Scopes.Any(x => !IntegrationScopes.All.Contains(x)) || input.ExpiresAt <= now ||
            ...
            if (user is not { Enabled: true } || user.Role is not ("Admin" or "Editor"))
                throw new CmsException(400, "INVALID_TOKEN_USER", "请选择已启用的管理员或编辑账号。");
            if (await repo.CountAsync<AccessToken>(x =>
                    x.UserId == user.Id && x.RevokedAt == null && x.ExpiresAt > now.UtcDateTime) >= 100)
            ...
            {
                Name = input.Name.Trim(), UserId = user.Id, Scopes = string.Join(' ', input.Scopes.Distinct().Order()),
                ExpiresAt = input.ExpiresAt.UtcDateTime, SecretHash = Hash(secret), CreatedAt = now.UtcDateTime
            };
    ...
    public Task<bool> RevokeAsync(string actor, string id)
    {
        return repository.WriteAsync(actor, "token.revoke", async repo =>
        {
    ...
    {
        if (value.Length != 101 || !value.StartsWith("cms_", StringComparison.Ordinal) || value[36] != '.' ||
            !value.AsSpan(4, 32).ToString().All(c => c is >= '0' and <= '9' or >= 'a' and <= 'f') ||
            !value.AsSpan(37).ToString().All(c => c is >= '0' and <= '9' or >= 'a' and <= 'f')) return null;
        var row = await repository.FindAsync<AccessToken>(value[4..36]);
        if (row == null || !CryptographicOperations.FixedTimeEquals(Encoding.ASCII.GetBytes(row.SecretHash),
                Encoding.ASCII.GetBytes(Hash(value[37..])))) return null;
        try
        {
            await RequireAsync(row.Id);
        }
        catch (CmsException e) when (e.Status == 401)
        {
            return null;
        }
    ...
    public async Task<AccessToken> RequireAsync(string id, string? scope = null)
    {
        ...
        if (token == null || token.RevokedAt != null || token.ExpiresAt <= clock.GetUtcNow().UtcDateTime ||
            user is not { Enabled: true } || user.Role is not ("Admin" or "Editor"))
            throw new CmsException(401, "INVALID_ACCESS_TOKEN", "访问令牌无效、已过期或已撤销。");
        if (scope != null && !token.Scopes.Split(' ').Contains(scope))
            throw new CmsException(403, "INSUFFICIENT_SCOPE", "访问令牌没有执行此操作的权限。");
        return token;
    }

    private AccessTokenView View(AccessToken token, CmsUser? user)
    {
        return new AccessTokenView(token.Id, token.Name, token.UserId, user?.DisplayName ?? "已删除账号",
            token.Scopes.Split(' '),
            token.CreatedAt, token.ExpiresAt, token.LastUsedAt, token.RevokedAt,
            token.RevokedAt != null ? "revoked" :
            token.ExpiresAt <= clock.GetUtcNow().UtcDateTime ? "expired" :
            user is not { Enabled: true } ? "disabled" : "active");
    }
