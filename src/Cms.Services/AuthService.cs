...
public sealed class AuthService(CmsRepository repository, IMapper mapper, LoginProtection protection)
{
    ...
    public async Task<CmsUser> LoginAsync(LoginInput input, string? browser)
    {
        ...
        if (input.Password is not { Length: <= 200 } || hasher.VerifyHashedPassword(candidate, candidate.PasswordHash, input.Password) == PasswordVerificationResult.Failed || user is not { Enabled: true })
            throw new CmsException(401, "INVALID_CREDENTIALS", "账号或密码错误。");
        protection.Succeeded(name);
        return user;
    }

    private static readonly Lazy<string> DummyHash = new(() => new PasswordHasher<CmsUser>().HashPassword(new CmsUser(), Guid.NewGuid().ToString()));

    /// <summary>Validate a session against live account state.</summary>
    public Task<CmsUser?> FindAsync(string id) => repository.FindAsync<CmsUser>(id);
    /// <summary>Map an account without credentials.</summary>
    public UserView View(CmsUser user) => mapper.Map<UserView>(user);
    /// <summary>List safe account information.</summary>
    public async Task<IReadOnlyList<UserView>> ListAsync() => (await repository.ListAsync<CmsUser>()).Select(View).ToList();

    /// <summary>Initialize the first administrator only; never replace existing accounts.</summary>
    public Task<UserView> InitializeAsync(string username, string password) => repository.WriteAsync("operator", "initialize", async repo =>
    {
        var existing = await repo.FirstAsync<CmsUser>(x => x.Role == "Admin" && x.Enabled);
        if (existing != null) { repo.SetAuditTarget("user", existing.Id, existing.DisplayName); return View(existing); }
        ...
        if (await repo.FindAsync<SiteSettings>("site") == null) await repo.InsertAsync(new SiteSettings { Id = "site" });
        return View(user);
    });

    /// <summary>Create or update an account and revoke old sessions.</summary>
    public Task<UserView> SaveAsync(string actor, string? id, UserInput input) => repository.WriteAsync(actor, "user.save", async repo =>
    {
        ...
        if (existing?.Role == "Admin" && existing.Enabled && (input.Role != "Admin" || !input.Enabled)) await ProtectLastAdmin(repo);
        if (existing != null) { incoming.Id = existing.Id; incoming.CreatedAt = existing.CreatedAt; incoming.PasswordHash = existing.PasswordHash; }
        if (!string.IsNullOrEmpty(input.Password)) incoming.PasswordHash = hasher.HashPassword(incoming, input.Password);
        if (incoming.PasswordHash.Length == 0) throw new CmsException(400, "PASSWORD_REQUIRED", "新账号必须设置密码。");
        if (existing == null) await repo.InsertAsync(incoming); else await repo.UpdateAsync(incoming);
    ...
    public Task<bool> DeleteAsync(string actor, string id) => repository.WriteAsync(actor, "user.delete", async repo =>
    {
        var user = await repo.FindAsync<CmsUser>(id) ?? throw Missing();
        if (user.Role == "Admin" && user.Enabled) await ProtectLastAdmin(repo);
        repo.SetAuditTarget("user", user.Id, user.DisplayName);
        await repo.DeleteAsync<CmsUser>(id); return true;
    });

    /// <summary>Rotate a password after verifying the current password.</summary>
    public Task<bool> ChangePasswordAsync(string id, PasswordInput input) => repository.WriteAsync(id, "password.change", async repo =>
    {
        var user = await repo.FindAsync<CmsUser>(id) ?? throw Missing();
        if (input.CurrentPassword == null || hasher.VerifyHashedPassword(user, user.PasswordHash, input.CurrentPassword) == PasswordVerificationResult.Failed)
        ...
        repo.SetAuditTarget("user", user.Id, user.DisplayName);
        await repo.UpdateAsync(user); return true;
    });

    private static CmsUser Build(UserInput input)
    {
        var name = (input.Username ?? "").Trim().ToLowerInvariant();
        if (!System.Text.RegularExpressions.Regex.IsMatch(name, "^[a-z0-9][a-z0-9._-]{2,63}$") || string.IsNullOrWhiteSpace(input.DisplayName) || input.DisplayName.Length > 100 || input.Role is not ("Admin" or "Editor"))
        ...
        return new CmsUser { Username = name, DisplayName = input.DisplayName.Trim(), Role = input.Role, Enabled = input.Enabled };
    }
    private static void ValidatePassword(string? value)
    {
        if (value is not { Length: >= 12 and <= 200 }) throw new CmsException(400, "INVALID_PASSWORD", "密码需为 12–200 个字符。");
    }
    ...
    private static CmsException Missing() => new(404, "NOT_FOUND", "账号不存在。");
}
