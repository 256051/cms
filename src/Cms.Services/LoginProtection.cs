...
public sealed class LoginProtection(TimeProvider clock) : IDisposable
{
    ...
    {
        var answer = new string(Enumerable.Range(0, 6).Select(_ => (char)('2' + RandomNumberGenerator.GetInt32(8)))
            .ToArray());
        var item = new Challenge(RandomNumberGenerator.GetHexString(64),
            SHA256.HashData(Encoding.UTF8.GetBytes(answer)), clock.GetUtcNow() + ChallengeLifetime);
        lock (gate)
        {
            Store(challenges, browser, item, ChallengeLifetime);
        }

        return new CaptchaView(item.Id,
            "data:image/svg+xml;base64," + Convert.ToBase64String(Encoding.UTF8.GetBytes(Render(answer))),
    ...
    public void BeginAttempt(string name, string? browser, LoginInput input)
    {
        lock (gate)
        {
            ...
            if (challenge.Expires <= clock.GetUtcNow() || input.CaptchaCode is not { Length: <= 16 } ||
                !CryptographicOperations.FixedTimeEquals(challenge.AnswerHash,
                    SHA256.HashData(Encoding.UTF8.GetBytes(input.CaptchaCode.Trim()))))
                throw InvalidCaptcha();
    ...
    public void Succeeded(string name)
    {
        lock (gate)
        {
            attempts.Remove(AccountKey(name));
        }
    }

    private static string AccountKey(string name)
    {
        return Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(name)));
    }

    private static CmsException InvalidCaptcha()
    {
        return new CmsException(400, "CAPTCHA_INVALID", "验证码错误或已失效，请刷新后重试。");
    }
    ...
    private static void Store<T>(MemoryCache cache, string key, T value, TimeSpan lifetime) where T : class
    {
        cache.Set(key, value,
            new MemoryCacheEntryOptions
                { Size = 1, AbsoluteExpirationRelativeToNow = lifetime, Priority = CacheItemPriority.NeverRemove });
    ...
    private static string Render(string answer)
    {
        var svg = new StringBuilder(
            "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"200\" height=\"64\" viewBox=\"0 0 200 64\"><rect width=\"200\" height=\"64\" rx=\"6\" fill=\"#f1f5f9\"/>");
        for (var i = 0; i < 16; i++)
            svg.Append(
                $"<path d=\"M{RandomNumberGenerator.GetInt32(200)} {RandomNumberGenerator.GetInt32(64)}L{RandomNumberGenerator.GetInt32(200)} {RandomNumberGenerator.GetInt32(64)}\" stroke=\"#cbd5e1\" fill=\"none\"/>");
        for (var i = 0; i < answer.Length; i++)
        {
            svg.Append(
                $"<path fill=\"#1e3a5f\" transform=\"translate({14 + i * 29} {16 + RandomNumberGenerator.GetInt32(7)}) rotate({RandomNumberGenerator.GetInt32(-9, 10)} 10 14)\" d=\"");
            var glyph = Glyphs[answer[i] - '2'];
            for (var pixel = 0; pixel < glyph.Length; pixel++)
                if (glyph[pixel] == '1')
                    svg.Append($"M{pixel % 5 * 4} {pixel / 5 * 4}h4v4h-4z");
            svg.Append("\"/>");
        }
