...
public sealed class ThemeOptionsValidator : AbstractValidator<ThemeOptions>
{
    ...
    public ThemeOptionsValidator()
    {
        ...
        RuleFor(x => x.HeroTitle).NotNull().MaximumLength(100).Must(x => Plain(x, false))
            .WithMessage("首页标题最多 100 字，只能使用纯文本。");
        RuleFor(x => x.HeroDescription).NotNull().MaximumLength(500).Must(x => Plain(x, true))
            .WithMessage("首页介绍最多 500 字，只能使用纯文本。");
    }

    private static bool Plain(string? value, bool multiline)
    {
        return value != null && !value.Any(c =>
            c is '<' or '>' || (char.IsControl(c) && !(multiline && c is '\r' or '\n' or '\t')));
    }
}
...
public sealed class ThemeService(CmsRepository repository, ThemeOptionsValidator validator)
{
    ...
    private static ThemeDefinition Define(string id, string name, string description, string color, string title)
    {
        ...
        return new ThemeDefinition(id, name, description, $"/themes/{id}.png", title, defaults, defaults);
    }

    private static ThemeDefinition Definition(string id)
    {
        return Catalogue.FirstOrDefault(x => x.Id == id) ?? throw new CmsException(400, "INVALID_THEME", "请选择系统预置的主题。");
    }

    private static Dictionary<string, ThemeOptions> Profiles(ThemeState state)
    {
        return JsonSerializer.Deserialize<Dictionary<string, ThemeOptions>>(state.ProfilesJson) ?? [];
    }

    private static ThemesView View(ThemeState state)
    {
        var profiles = Profiles(state);
        return new ThemesView(state.ActiveThemeId, state.Version,
            Catalogue.Select(x => x with { Options = profiles.GetValueOrDefault(x.Id) ?? x.Defaults }).ToArray());
    }
    ...
    private async Task<ThemeView> ResolveAsync(string id, ThemeOptions options)
    {
        ...
        var site = await repository.FindAsync<SiteSettings>("site") ??
                   throw new CmsException(503, "NOT_READY", "站点尚未初始化。");
        var title = definition.DefaultHeroTitle;
        return new ThemeView(id,
            options with
            {
                AccentColor = options.AccentColor.ToUpperInvariant(),
                HeroTitle = string.IsNullOrWhiteSpace(options.HeroTitle) ? title : options.HeroTitle,
                HeroDescription = string.IsNullOrWhiteSpace(options.HeroDescription)
                    ? site.Description
                    : options.HeroDescription
            });
    }

    /// <summary>Read all saved profiles and the current revision for administrators.</summary>
    public async Task<ThemesView> ListAsync()
    {
        return View(await repository.FindAsync<ThemeState>("site") ??
                    throw new CmsException(503, "NOT_READY", "请先升级数据库。"));
    }
    ...
    public Task<ThemeView> PreviewAsync(string themeId, string? accentColor, string? heroTitle, string? heroDescription)
    {
        ...
        return ResolveAsync(themeId,
            new ThemeOptions(accentColor ?? theme.Defaults.AccentColor, heroTitle ?? "", heroDescription ?? ""));
    }
    ...
    public async Task<ThemesView> ApplyAsync(string actor, ThemeApplyInput input)
    {
        ...
        {
            var state = await repo.FindAsync<ThemeState>("site") ??
                        throw new CmsException(503, "NOT_READY", "请先升级数据库。");
            var profiles = Profiles(state);
