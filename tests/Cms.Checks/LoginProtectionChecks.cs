...
internal static class LoginProtectionChecks
{
    internal static void Run()
    {
        ...
        LoginInput Issue(string owner = browser)
        {
            return Input(protection.Create(owner));
        }
        ...
        try
        {
            ...
            throw new Exception("Cooldown bypassed");
        }
        ...
        Parallel.For(0, 12, i =>
        {
            ...
            try
            {
                protection.BeginAttempt(name, owner, input);
                outcomes.Add("allowed");
            }
        ...
        if (outcomes.Count(x => x == "allowed") != 5 || outcomes.Count(x => x == "LOGIN_LOCKED") != 7)
            throw new Exception("Concurrent account limit failed");
        var single = Issue("once");
        outcomes.Clear();
        Parallel.For(0, 12, _ =>
        {
            try
            {
                protection.BeginAttempt("single", "once", single);
                outcomes.Add("allowed");
            }
            catch (CmsException e)
            {
        ...
        if (outcomes.Count(x => x == "allowed") != 1 || outcomes.Count(x => x == "CAPTCHA_INVALID") != 11)
            throw new Exception("Challenge replay race");
        Console.WriteLine(
            "PASS: browser binding, refresh, wrong/used/expired challenges, 5-attempt limit, cooldown expiry, success reset and concurrent replay/account limits");
    }
    ...
    private static LoginInput Input(CaptchaView view)
    {
        ...
        var glyphs = (string[])typeof(LoginProtection).GetField("Glyphs", BindingFlags.NonPublic | BindingFlags.Static)!
            .GetValue(null)!;
        ...
        var answer = string.Concat(Regex.Matches(svg, "<path fill=\"[^\"]+\" transform=\"[^\"]+\" d=\"([^\"]+)\"")
            .Select(m =>
            {
                var bits = Enumerable.Repeat('0', 35).ToArray();
                foreach (Match pixel in Regex.Matches(m.Groups[1].Value, @"M(\d+) (\d+)h4v4h-4z"))
                    bits[int.Parse(pixel.Groups[2].Value) / 4 * 5 + int.Parse(pixel.Groups[1].Value) / 4] = '1';
                return (Array.IndexOf(glyphs, new string(bits)) + 2).ToString();
            }));
        if (answer.Length != 6) throw new Exception("Invalid image");
        return new LoginInput("test-user", "Password!123456", view.Id, answer);
    }
