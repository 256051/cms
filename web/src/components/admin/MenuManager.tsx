...
export default function MenuManager() {
  ...
  const { data, error, setError, reload, loading } = useLoad<Menu[]>("admin/menu");
  ...
  function edit(item?: Menu, parentId = "") {
    ...
    changes.markSaved(); setEditing(item); setDraft(item ? { ...item } : { ...empty, parentId }); setError(""); setSuccess("");
    requestAnimationFrame(() => heading.current?.focus());
  }
  return <>
    <Heading title="导航菜单" description="按层级组织主导航。选择站内内容可自动生成名称和链接，序号越小越靠前。">
      <button disabled={busy} onClick={() => edit()}><Plus size={16} />新增菜单项</button>
    </Heading>
    ...
    <div className="management-grid menu-management">
      <section className="panel menu-editor">
        <h2 ref={heading} tabIndex={-1}>{editing ? "编辑菜单项" : "添加菜单项"}</h2>
        <form onSubmit={async e => {
          e.preventDefault(); setBusy(true); setError(""); setSuccess("");
          try {
            await api("admin/menu" + (editing ? "/" + editing.id : ""), editing ? "PUT" : "POST", draft);
            changes.markSaved(); setEditing(undefined); setDraft({ ...empty }); await reload(); setSuccess("菜单项已保存，网站导航已更新。");
          } catch (e) { setError((e as Error).message); } finally { setBusy(false); }
        }}>
          <fieldset className="form-fields" disabled={busy || loading}>
            <label>上级菜单项<select aria-label="上级菜单项" value={draft.parentId} onChange={e => change({ parentId: e.target.value })}>
            ...
            <label>链接<input name="url" required maxLength={500} readOnly={draft.type !== "custom"} value={draft.url} placeholder="/pages/about、# 或 https://example.com" onChange={e => change({ url: e.target.value })} /></label>
            ...
            <label>排序<input name="sort" aria-label="排序" aria-describedby="menu-sort-help" type="number" required min={-2147483648} max={2147483647} value={draft.sort} onChange={e => change({ sort: Number(e.target.value) })} /><small id="menu-sort-help">仅比较同级菜单。支持最多 5 级菜单。</small></label>
      ...
      <section className="panel menu-tree-panel" aria-label="菜单层级">
        <h2>主导航</h2><p className="muted">首页和搜索入口由主题提供，下方为自定义导航。</p>
        <ul className="menu-tree">
          {rows.map(({ item, depth }) => <li key={item.id} style={{ marginLeft: depth * 16 }}>
            <div className="menu-tree-row">
              <div className="menu-item-info"><strong>{depth > 0 && <CornerDownRight size={15} />}{item.label}{item.openInNewTab && <ArrowUpRight size={15} aria-label="新窗口" />}</strong><span className="muted">{menuTypes[item.type]} · 排序 {item.sort} · 第 {depth + 1} 级</span><span className="menu-item-url">{item.url}</span>{!item.available && <span className="badge">内容不可用，前台隐藏</span>}</div>
              <div className="row-actions">
                ...
                <button className="icon-button" disabled={busy || depth >= 4} aria-label={"新增子菜单 " + item.label} onClick={() => edit(undefined, item.id)}><Plus size={16} /></button>
                <button className="icon-button danger-text" disabled={busy} aria-label={"删除 " + item.label} onClick={async () => {
                  ...
                  if (!confirm(`删除菜单项“${item.label}”？`)) return;
                  setBusy(true); setError("");
                  try { await api(`admin/menu/${item.id}?version=${item.version}`, "DELETE"); if (editing?.id === item.id) { changes.markSaved(); setEditing(undefined); setDraft({ ...empty }); } await reload(); }
                  catch (e) { setError((e as Error).message); } finally { setBusy(false); }
        ...
        {!loading && !error && rows.length === 0 && <p className="empty-state">还没有导航菜单，先添加一个菜单项。</p>}
      </section>
    </div>
  </>;
}
